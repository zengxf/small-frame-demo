 ...
 */
@Slf4j
public class EnterpriseClientTest implements Constant {
    ...
    static void main() {
        ...
        // clientConfig.setProperty("hazelcast.enterprise.license.key", "TrialLicense#10Nodes#xx"); // 不需要设置 key

        // 连接到集群
        HazelcastInstance client = HazelcastClient.newHazelcastClient(clientConfig);

        // 列出所有集群成员
        client.getCluster().getMembers().forEach(member -> {
            Address address = member.getAddress();
            log.info("集群成员: [{}] (UUID: {})", address, member.getUuid());
        });
